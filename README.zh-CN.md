...
用法最后会变成一列在动手**之前**敲下的名字。但 harness 其实一直在写一个准确的
标题（Claude Code 在后台生成，并通过 ACP 上报），所以网关会拿它去改话题名。

```
/title                  →  这个话题上次被我起的名字，以及是谁起的
/title ask 超时          →  自己起名，之后智能体不再改它
/title auto             →  把命名权交回智能体
```

你手动设的名字会被**钉住**：那个会话的自动改名就停了。因为一条命令的效果被下一轮
悄悄推翻，跟命令本身坏掉是分不清的。想整体关掉自动那半边，用
`platforms.<id>.autoRenameThread: false`。

两个限制值得知道。标题是后台任务生成、在一轮结束时读取的，所以它描述的是**上一轮**
而不是应用它的那一轮 —— 一个话题里的第一次对话通常还没有标题。另外 Telegram 没有
任何接口能读回话题当前的名字，所以你直接在 Telegram 界面里改的名字这边是看不见的，
会被下一个新标题覆盖；要保住自己起的名字，用 `/title` 或那个开关。

只有走 ACP 的 harness 会上报标题（`claude` 实测会报，`opencode` 的协议里有这个
字段），所以用 `dsh` 或 `agy` 时，话题名不会自己变，除你自己设。
